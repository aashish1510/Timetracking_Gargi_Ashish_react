...
export const TaskForm = () => {
  ...
  const ProjectChangeHandler = async (e) => {
    ...
    await axios.get(`http://localhost:4000/modulebyproject/${e.target.value}`).then((res) => {
    ...
    <div>
      <form onSubmit={submit}>
          ...
          <select class="form-control" onChange={ModuleChangeHandler}>
            {
              Module.map((row) => {
                return (
                  <option value={row._id}>{row.modulename}</option>
                )
              })
            }
          </select>
          ...
          <select class="form-control" onChange={PriorityChangeHandler}>
            {
              priority.map((row) => {
                return (
                  <option value={row._id}>{row.priorityName}</option>
                )
              })
            }
          </select>
        ...
        <div class="form-group">
          ...
          <input type="number" class="form-control" id="exampleInputPassword1" placeholder="Enter Estimated Hours" onChange={(e) => { TotalHoursChangeHandler(e) }} />
        </div>
        <div class="form-group">
